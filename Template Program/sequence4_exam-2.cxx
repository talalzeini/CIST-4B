...
#include <cstdlib>
#include "node2.h"     // Provides size_t.
...
// **************************************************************************
bool test_basic(const sequence<int>& test, size_t s, bool has_cursor)
{
    bool answer;
...
// **************************************************************************
bool test_items(sequence<int>& test, size_t s, size_t i, double items[])
{
    bool answer = true;
...
// **************************************************************************
bool correct(sequence<int>& test, size_t size, size_t cursor_spot, double items[])
{
...
int test2( )
{
    ...
    for (i = 4; i <= TESTSIZE; i++)//cannot find 40...but...if i is starting at 4, it never will find that index
        test.attach(i*10);
...
int test4( )
{
    ...
    //////////////////////////////
    sequence<int> copy4(original);//<---DA FAQ!?! This seems to be causing a problem, but why?
    ////////////////////////////
    original.start( );
    original.advance( );
    original.remove_current( ); // Delete 2 from original, but not the copy.
